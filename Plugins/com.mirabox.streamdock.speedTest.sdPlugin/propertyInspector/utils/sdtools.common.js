...
function connectElgatoStreamDeckSocket(inPort, inUUID, inRegisterEvent, inInfo, inActionInfo) {
    ...
    console.log(inUUID, inInfo, inActionInfo);
    actionInfo = JSON.parse(inActionInfo); // cache the info
    appInfo = JSON.parse(inInfo);
    websocket = new WebSocket('ws://127.0.0.1:' + inPort);

    addDynamicStyles(appInfo.colors);
...
function isPositiveIntegerByRegexSimple(value) {
...
function validatePositiveInteger(inputElement) {
    const value = inputElement.value.trim();
    
    
    if (isPositiveIntegerByRegexSimple(value)) {
        debouncedSetSettings(); // 合法输入，更新设置
    } else {
        inputElement.value = ""; // 非法输入，清空
        debouncedSetSettings(); // 合法输入，更新设置
    }
}

const debouncedSetSettings = debounce(setSettings, 300); // 300毫秒延迟
...
function setSettings() {
    ...
    Array.prototype.forEach.call(elements, function (elem) {
        ...
        if (elem.classList.contains("sdCheckbox")) { // Checkbox
            ...
            var valueField = elem.getAttribute("sdValueField");
            console.log('sdValueField',valueField,elem,elem.value)
...
async function i18n() {
    ...
    const lang = await new Promise(resolve => {
        ...
        req.open('GET', `../../${appInfo.application.language}.json`);
        req.send();
