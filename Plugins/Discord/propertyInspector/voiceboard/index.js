...
$dom.search.on('input', (e) => {
  ...
  Object.keys($settings.sounds)
    .reverse()
    .forEach((guildId) => {
      const filteredSounds = $settings.sounds[guildId].filter((sound) => sound.name.includes(e.target.value));
  ...
  soundItem.forEach((item) => {
    item.addEventListener('click', (e) => {
      ...
      Object.assign($settings, {
        ...
        sound_id: e.target.getAttribute('soundId'),
        sounds: $settings.sounds, // 保留现有 sounds
        title: e.target.getAttribute('soundName'),
        emoji_name: e.target.getAttribute('emojiName'),
      });
    });
  });
};
    ...
    .forEach((key) => {
      ...
      sounds[key].forEach((item) => {
        ...
        if (settings.sound_id == item.sound_id) {
          div +=
            '<div class="soundItem active" key="' +
            key +
            '" soundId="' +
            item.sound_id +
            '" soundName="' +
            ...
            item.emoji_name +
            '">' +
            (item.emoji_name ? item.emoji_name : '') +
            item.name +
            '</div>';
        } else {
          div +=
            '<div class="soundItem" key="' +
            key +
            '" soundId="' +
            item.sound_id +
            '" soundName="' +
