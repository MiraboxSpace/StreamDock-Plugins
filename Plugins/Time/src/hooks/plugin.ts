...
export const usePluginStore = defineStore('pluginStore', () => {
  ...
  class Actions {
    ...
    static getActions = (action = '') => {
      return action ? this.list.filter((item) => item.action === action) : this.list;
    };
...
export const useWatchEvent = <T extends keyof MessageTypes>(type: T, MessageEvents: MessageTypes[T]) => {
  ...
  const Events: StreamDock.ActionMessage = {
    willAppear({ action, context, payload }) {
      const Actions = plugin.getActions(action);
      if (!Actions.find((item) => item.context === context)) {
        plugin.addAction(action, context, payload.settings);
      }
    },
