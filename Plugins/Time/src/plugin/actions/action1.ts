...
export default function (name: string) {
  ...
  const canvasFunc = (context: string) => {
    ...
    const themeFunc = {
      async theme1() {
        ...
        if (settings.select === '1') {
          ...
          // ctx.font = `${titleParameters.fontStyle == "Regular" ? "" : titleParameters.fontStyle} ${titleParameters.fontSize + 27}px '${titleParameters.fontFamily}'`;
          // ctx.fillText(`${h}:${m}`, 63, 75 + offset);
          // if (titleParameters.fontUnderline) {
          //   let textMetrics = ctx.measureText(`${h}:${m}`);
          //   let underlineHeight = 1;
          //   ctx.fillRect(63 - (textMetrics.width / 2), 77 + offset, textMetrics.width, underlineHeight);
          // }
      ...
      async theme2() {
        ...
        ctx.textAlign = 'center';
        ctx.fillStyle = instance.titleParameters.titleColor; //instance.titleParameters.titleColor
    ...
    (async () => {
      ...
      // instance.setTitle(settings.zone === 'system' ? 'System' : `UTC${settings.zone}`);
  ...
  useWatchEvent('action', {
    ...
    didReceiveSettings({ context }) {
      const settings = plugin.getAction(context).settings as Settings;
      if (settings.zone != zones[context]) {//如果改变了时区才同步更改标题，避免更改标题后又被覆盖为时区
        settings.title = settings.zone === 'system' ? 'SYS' : `UTC${settings.zone}`;
      }
      zones[context] = settings.zone;//记录时区用于下次判断是否改变
      plugin.getAction(context).setSettings(settings)
      canvasFunc(context);
    },
