...
<template>
  <div class="p-3">
    ...
    <div class="mb-3">
      <label class="block  font-medium text-white mb-1">{{ i18n["BrowserSourceName"] }}</label>
      <select
        class="w-full p-2 bg-[#3d3d3d] border border-[#4a4a4a] rounded text-white placeholder-gray-400 focus:border-[#505050] focus:outline-none transition-colors"
    ...
    <div class="mb-3 flex items-center">
      <input type="checkbox" v-model="property.settings.file" id="file"
      ...
      <label for="file" class="ml-2 block  text-white">{{ i18n["LoadLocalFile"] }}</label>
    </div>

    <div class="mb-3" v-if="!property.settings.file">
      <label class="block  font-medium text-white mb-1">{{ i18n["URL"] }}</label>
      <input type="text" placeholder="https://mirabox.key123.vip/home"
        class="w-full p-2 bg-[#3d3d3d] border border-[#4a4a4a] rounded text-white placeholder-gray-400 focus:border-[#505050] focus:outline-none transition-colors"
        v-model="property.settings.url">
    </div>

    <div class="mb-3" v-else>
      <label class="block  font-medium text-white mb-1">{{ i18n["LocalFile"] }}</label>
      <input type="file" accept=".html" id="localFile" class="hidden">
      <label for="localFile"
        class="inline-block w-full px-4 py-2 bg-[#3d3d3d] border border-[#4a4a4a] rounded-md cursor-pointer text-center">{{
        property.settings.localFile ? property.settings.localFile : i18n["ChooseHTMLFile"]
      }}</label>
    </div>
    ...
    <!-- <div class="mb-3 flex items-center">
      <input type="checkbox" id="mute"
        class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50"
        v-model="property.settings.mute">
      <label for="mute" class="ml-2 block  text-white">{{ i18n["MuteBrowserSource"] }}</label>
    </div> -->
