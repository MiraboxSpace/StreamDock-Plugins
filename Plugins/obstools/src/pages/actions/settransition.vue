...
useWatchEvent({
  ...
  sendToPropertyInspector(data) {
    console.log(data);
    if ("transitionList" in data.payload) {
      transitionList.value = data.payload.transitionList;
    }
  },
  ...
  <div class="p-3">
    <div class="mb-3">
      ...
      <select
        ...
        <option :value="item.value" v-for="item in transitionList">{{ item.key }}</option>
      </select>
    </div>
    <div class="mb-3">
      <label class="block  font-medium text-white mb-1">{{ i18n["Duration"] }}</label>
      <div class="flex">
        ...
        v-model="property.settings.durationChange">
      <label for="mute" class="ml-2 block  text-white">{{ i18n["Change transition duration"] }}</label>
      </div>
    </div>
    <div class="mb-3" v-show="property.settings.durationChange">
      <label class="block  font-medium text-white mb-1">{{ i18n["Duration(ms)"] }}</label>
      <input type="number" min="0"
        class="w-full p-2 bg-[#3d3d3d] border border-[#4a4a4a] rounded text-white placeholder-gray-400 focus:border-[#505050] focus:outline-none transition-colors"
        v-model="property.settings.duration">
    </div>
