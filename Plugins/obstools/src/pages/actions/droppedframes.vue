...
useWatchEvent({
  ...
  sendToPropertyInspector(data) {
    console.log(data.payload.framesTypeList);
    if("framesTypeList" in data.payload) {
      framesTypeList.value = data.payload.framesTypeList;
    }
  },
...
<template>
  <div class="p-3">
    ...
    <div class="mb-3">
      <label class="block  font-medium text-white mb-1">{{ i18n["Frames Type"] }}</label>
      <select
        ...
        v-model="property.settings.framesType">
        <option :value="item.value" v-for="item in framesTypeList">{{ item.name }}</option>
      </select>
    </div>
    <div class="mb-3">
      <label class="block  font-medium text-white mb-1">{{ i18n["Alert Color"] }}</label>
      <input type="color" 
      class="w-full bottom-0"
      v-model="property.settings.color" />
    </div>
    <div class="mb-3">
      <label class="block  font-medium text-white mb-1">{{ i18n["Minimum Threshold (frames)"] }}</label>
      <input 
      type="number" 
      class="w-full p-2 bg-[#3d3d3d] border border-[#4a4a4a] rounded text-white placeholder-gray-400 focus:border-[#505050] focus:outline-none transition-colors"
      v-model="property.settings.minimum_threshold" />
    </div>
    <div class="mb-3">
      {{ i18n["Minimum Threshold tips"] }}
    </div>
    <div class="mt-4">
      <button @click="logout"
