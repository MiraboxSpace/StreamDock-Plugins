...
std::shared_ptr<HSDAction> HSDSwitchAudioPlugin::GetOrCreateAction(const std::string& action, const std::string& context)
{
    ...
    if (action == "com.mirabox.streamdock.switchAudio.myaction1") {
...
void HSDSwitchAudioPlugin::DidReceiveGlobalSettings(const nlohmann::json& payload)
{
