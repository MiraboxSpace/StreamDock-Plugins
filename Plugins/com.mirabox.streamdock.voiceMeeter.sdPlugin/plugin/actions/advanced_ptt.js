    ...
    default: {
      ...
      stateChange: function (data, flag) {
        ...
        if (flag) {
          ...
          strCommand = key_release;
          sender.sendMidiCommand(release_midi);
        }
        log.info(strCommand);
        vmInstance.setOption(strCommand);
      },
      keypress_send_hotkey_action: (hotkey) => {
        plugin.advanced_ptt.default.stateChange({ context: hotkey.context, payload: { settings: plugin.advanced_ptt.data[hotkey.context] } }, true)
      },
      release_send_hotkey_action: (hotkey) => {
        plugin.advanced_ptt.default.stateChange({ context: hotkey.context, payload: { settings: plugin.advanced_ptt.data[hotkey.context] } }, false)
      },
    },
    ...
    async sendToPlugin({ payload, context }) {
      ...
      if ("type" in payload) {
        if (payload.type === "hotkey") {
          ...
          if (payload.id === "release_send_hotkey_action") {
            settings.release_send_hotkey = recordedHotkey.shortcut;
          }
