...
const $local = true, $back = false, $dom = {
    ...
    release_send_hotkey: $('#release_send_hotkey'),
    release_midi: $('#release_midi'),
    title: $('#title'),
...
const $propEvent = {
    ...
    didReceiveSettings(data) {
        ...
        $dom.key_release.value = $settings.key_release;
        $dom.release_send_hotkey.value = $settings.release_send_hotkey;
        $dom.release_midi.value = $settings.release_midi;
...
$dom.key_release.addEventListener('input', debounce(function() {
  $settings.key_release = this.value;
}, DEBOUNCE_TIME));

$dom.release_midi.addEventListener('input', debounce(function() {
  $settings.release_midi = this.value;
}, DEBOUNCE_TIME));
...
$dom.release_send_hotkey.addEventListener('click', function () {
  // Add specific logic for release_send_hotkey click if needed
  $websocket.sendToPlugin({type: "hotkey", id: "release_send_hotkey_action"})
});
