...
void HSDPlugin::PropertyInspectorDidAppear(const std::string& inAction, const std::string& inContext, const nlohmann::json& inPayload, const std::string& inDevice)
{
    auto action = GetOrCreateAction(inAction, inContext);
    if (!action) {
        HSDLogger::LogMessage("No action for PropertyInspectorDidAppear: " + inAction + " " + inContext);
        return;
    }
    action->PropertyInspectorDidAppear(inPayload);
}

void HSDPlugin::PropertyInspectorDidDisappear(const std::string& inAction, const std::string& inContext, const nlohmann::json& inPayload, const std::string& inDevice)
{
    auto action = GetOrCreateAction(inAction, inContext);
    if (!action) {
        HSDLogger::LogMessage("No action for PropertyInspectorDidAppear: " + inAction + " " + inContext);
