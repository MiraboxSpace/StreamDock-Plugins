...
std::shared_ptr<HSDAction> HSDExamplePlugin::GetOrCreateAction(const std::string& action, const std::string& context)
{
    ...
    if (action == "com.mirabox.streamdock.battery.action1") {
        auto impl = std::make_shared<GhubAction>(
            mConnectionManager,
            action,
            context
            );
    ...
    else if (action == "com.mirabox.streamdock.battery.action3") {
        auto impl = std::make_shared<SynapseAction>(
            mConnectionManager,
            action,
            context
        );
    ...
    else if (action == "com.mirabox.streamdock.battery.action4") {
        auto impl = std::make_shared<XboxAction>(
            mConnectionManager,
            action,
            context
        );
