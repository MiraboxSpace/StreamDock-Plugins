        ...
        if (data.event === 'didReceiveSettings') {
            $settings = new Proxy(data.payload.settings, {
                ...
                set(target, property, value) {
                    ...
                    // saveData也是setSetings,然后这里代理了data.payload.settings的get和set
                    // get里面直接返回了要获取的值所以不用管基本没变化和原本的效果是一样的
                    // 主要是set这里面除了设置值还额外发了一个saveData也就是setSetings
                    $websocket.saveData(data.payload.settings);
                }
            });
            if (!$back) $dom.main.style.display = 'block';
        }
        $propEvent[data.event]?.(data.payload);//这里传递进去的是payload所以那边就不用.payload了
    };
